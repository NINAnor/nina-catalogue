...
def soup_save_as_metadata(soup, metadata):
    ...
    for identifier in dataset.find_all("alternateIdentifier"):
        source = MetadataIdentifier.Type.IPT if "ipt.nina.no" in identifier.text else MetadataIdentifier.Type.GBIF
    ...
    for keyword_set in dataset.find_all("keywordSet"):
        ...
        for kw in keyword_set.find_all("keyword"):
            k, _ = Keyword.objects.get_or_create(name=text_or_null(kw), definition=definition, description=thesaurus)
...
def to_metadata(xml_path: pathlib.Path, dataset: Dataset):
    with open(str(xml_path)) as eml:
        ...
        with transaction.atomic():
            ...
            soup_save_as_metadata(soup, metadata)
